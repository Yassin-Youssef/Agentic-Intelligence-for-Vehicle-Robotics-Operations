...
class ReporterAgent:
    def generate_report(self, analysis_results: Dict[str, Any], fleet_status: Dict[str, Any]) -> str:
        ...
        for vehicle_id, vehicle_results in analysis_results.items():
            #Health status
            status_info = fleet_status.get(vehicle_id, {})
            vehicle_status = status_info.get("status", "Unkown")
            report_lines.append("=" * 30)
            report_lines.append(f"Vehicle ID: {vehicle_id}")
            #show health status clearly
            report_lines.append(f"Health Status: {vehicle_status}")
            report_lines.append("="*30)
